import { Building2, CheckCircle2, HelpCircle, Loader2, Mail, MessageSquare, User, X } from 'lucide-react';
...
const EMAILJS_FAQ_SERVICE_ID = import.meta.env.VITE_EMAILJS_FAQ_SERVICE_ID || import.meta.env.VITE_EMAILJS_CONTACT_SERVICE_ID;
const EMAILJS_FAQ_TEMPLATE_ID = import.meta.env.VITE_EMAILJS_FAQ_TEMPLATE_ID;
const EMAILJS_FAQ_PUBLIC_KEY = import.meta.env.VITE_EMAILJS_FAQ_PUBLIC_KEY || import.meta.env.VITE_EMAILJS_CONTACT_PUBLIC_KEY;

async function submitFaqQuestion(formData) {
  if (!EMAILJS_FAQ_SERVICE_ID || !EMAILJS_FAQ_TEMPLATE_ID || !EMAILJS_FAQ_PUBLIC_KEY) {
    throw new Error('FAQ form is not configured yet.');
  }
  ...
  const response = await emailjs.send(
    EMAILJS_FAQ_SERVICE_ID,
    EMAILJS_FAQ_TEMPLATE_ID,
    templateParams,
    { publicKey: EMAILJS_FAQ_PUBLIC_KEY }
  );
...
export default function FaqQuestionModal() {
  const { faqOpen, closeFaq } = useModal();
  ...
  useEffect(() => {
    if (!faqOpen) return undefined;
    const onKey = (e) => e.key === 'Escape' && closeFaq();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('FAQ question failed:', err);
      setStatus('error');
      ...
      if (status === 400 || String(text).toLowerCase().includes('invalid')) {
        friendly = 'There was a problem sending your question. Please check your EmailJS service/template settings or contact support@globalhris.com.';
      }
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true" aria-label="Ask a question">
      <button type="button" aria-label="Close" onClick={closeFaq} className="absolute inset-0 bg-navy-950/80 backdrop-blur-sm" />
      ...
      >
        ...
        {status === 'success' ? (
          <div className="py-6 text-center">
            ...
            <h3 className="font-display text-xl font-bold text-navy-900 dark:text-white">Question Sent!</h3>
            <p className="mt-2.5 text-sm text-slate-500 dark:text-slate-400">
              {form.fullName ? `Thanks, ${form.fullName.split(' ')[0]}!` : 'Thanks!'} Your question has been received successfully.
            </p>
          ...
          <>
            ...
            <h3 className="mt-2 font-display text-xl font-bold text-navy-900 dark:text-white">Still have questions?</h3>
            <p className="mt-1.5 text-sm text-slate-500 dark:text-slate-400">Submit your question and our team will get back to you.</p>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              <label className="block">
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Your Question</span>
                <div className="mt-1.5 relative">
              ...
              <button type="submit" disabled={status === 'submitting'} className="btn-primary w-full mt-2 disabled:opacity-70">
                {status === 'submitting' ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" /> Sending your question…
                  </>
                ) : (
                  'Submit Question'
                )}
