...
`src/components/FreeTrialModal.jsx` collects Full Name, Work Email, Company Name, Phone, Employee Count and Job Role. On submit it sends the lead to sales@globalhris.com via **EmailJS**, or — when `VITE_TRIAL_API_URL` is set — POSTs to that trial backend's `/api/trial/signup` endpoint (see `IMPLEMENTATION.md` §13). The success state offers a **"Start Free Trial"** button that opens the in-app trial landing page (`TrialVerifyPage.jsx`) and trial preview (`TrialDemoPage.jsx`) in the same tab. Set the real EmailJS / trial-API values in a git-ignored `.env` (see `.env.example`).
